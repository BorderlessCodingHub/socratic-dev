...
const DESIGN_SYS = `Você resolve um desafio de SYSTEM DESIGN (arquitetura). Responda APENAS com JSON válido (sem markdown):
{ "nodes": [{ "id": string, "label": string, "type": string }], "edges": [{ "from": string, "to": string }] }
- nodes: 4 a 8 componentes. O "type" DEVE ser um de: "client", "gateway", "service", "database", "cache", "queue", "storage", "external".
- label curto e específico (ex.: "App mobile", "API de pedidos", "Postgres (escrita)", "Réplica de leitura", "Redis", "Fila de eventos", "S3").
- edges: o fluxo dos dados entre os nós (from/to = ids de nodes existentes).
- Português do Brasil nos labels.`
